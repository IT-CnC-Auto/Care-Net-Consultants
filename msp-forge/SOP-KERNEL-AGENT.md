...
- The legislation register is public. `msp_public_instrument_register` (migration 046) lists every verified instrument with its full citation, gazette reference, verification date, next review and the industries it applies to. The Build your Plan page reads it live. The dated PDF and CSV release 1.0.0 were removed from vercel/downloads/ on 23/09/2026, because they still listed the repealed NIHL Regulations, 2003 and Environmental Regulations for Workplaces, 1987 as verified (HSF-7); their old addresses redirect to /shop.html#law (vercel.json and server/serve.js), and HSF-10 is closed. The CSV is kept, unpublished, as the File library seed source at hsf/sources/CNC-Legislation-Register-v1.0.0.csv. A new release of the register means a new dated file, generated from the view and published under a new address, and the release line on the page restored.
