// CNC HSF FORGE | HSF-ADS-01 v1.1.0 | First party Bee-Inspect banner events and attribution
// Built to hsf/BUILD-CONTRACT.md 16.3 (Bee-Inspect P1) and section A4 of
// hsf/BEE-INSPECT-BUILD-PROMPT.md; attribution_seen added in P2 (A3, "keep
// UTM through magic link; store on first sign in").
//
//   POST /api/hsf-events  {event, ad_id, variant, page, industry, f_band}
//     -> hsf_ad_event_record(p) (migration 057), service role only
//   POST /api/hsf-events  {event: 'attribution_seen', page, utm_source,
//                          utm_medium, utm_campaign, utm_content, utm_term}
//     -> hsf_attribution_record(p) (migration 058), service role only. Sent
//        once by the File builder at the first sign in of a visit that
//        arrived from a tagged link. Each tag is 1 to 64 letters, digits,
//        dots, underscores or hyphens (no @, so no email address fits); at
//        least one tag. Until 058 is applied the event is dropped (202).
...
// P2: the campaign tags of the link that brought a visit, at its first sign in.
const ATTRIBUTION_EVENT = 'attribution_seen';
const UTM_KEYS = ['utm_source', 'utm_medium', 'utm_campaign', 'utm_content', 'utm_term'];
const ATTRIBUTION_KEYS = ['event', 'page'].concat(UTM_KEYS);
const ATTRIBUTION_PAGES = ['/hsf-builder', '/health-and-safety-file', '/bee-inspect', '/get-app', '/portal'];
...
// A clean attribution, or { error }.
function validateAttribution(b) {
  for (const k of Object.keys(b)) {
    if (!ATTRIBUTION_KEYS.includes(k)) return { error: 'unknown field ' + String(k).slice(0, 40) };
  }
  for (const k of ATTRIBUTION_KEYS) {
    const v = b[k];
    if (v !== undefined && v !== null && typeof v !== 'string') return { error: k + ' must be text' };
  }
  if (!ATTRIBUTION_PAGES.includes(b.page)) return { error: 'unknown page' };
  const out = { event: ATTRIBUTION_EVENT, page: b.page };
  let any = false;
  for (const k of UTM_KEYS) {
    const v = b[k] === undefined || b[k] === null || b[k] === '' ? null : b[k];
    if (v !== null && !UTM_RE.test(v)) return { error: 'bad ' + k };
    out[k] = v;
    if (v !== null) any = true;
  }
  if (!any) return { error: 'no campaign tag' };
  return { event: out, attribution: true };
}

// A clean event, or { error }.
function validate(b) {
  if (b && b.event === ATTRIBUTION_EVENT) return validateAttribution(b);
  for (const k of Object.keys(b)) {
...
module.exports = async (req, res) => {
  ...
  const fn = v.attribution ? 'hsf_attribution_record' : 'hsf_ad_event_record';
  try {
    const r = await rpc(fn, { p: v.event });
    res.status(202).json({ accepted: !!(r && r.accepted === true) });
  } catch (err) {
    // 057 or 058 not applied yet, or the database refused: the event is
    // dropped, never a 500 to the page. The log line names the gap, never the event.
    const m = /^hsf_(?:ad_event|attribution)_record failed: (\d{3})/.exec((err && err.message) || '');
...
module.exports.ATTRIBUTION_EVENT = ATTRIBUTION_EVENT;
